...
- **Simulator crash handling**: cuTensorNet occasionally fails with `CUTENSORNET_STATUS_INVALID_VALUE` on specific (circuit, layout) combinations. Crashed terminal-reward computations are caught, the whole episode is **dropped from the replay buffer** (no synthetic `reward=0` is injected, which would bias the agent against layouts that are actually fine), and the event is appended to `runs/<RUN>/<backend>_crashes.csv` with circuit name, layout, and error type for later pattern analysis.
